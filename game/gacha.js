...
.legend{display:flex;justify-content:center;gap:16px;flex-wrap:wrap;background:rgba(0,0,0,0.2);border-radius:12px;padding:8px 16px;margin-bottom:20px;font-size:14px}
...
.star{font-size:28px;display:inline-block;animation:fadeIn .3s}
@keyframes fadeIn{from{opacity:0;transform:scale(.8)}to{opacity:1;transform:scale(1)}}
.btn{background:#8b5cf6;border:none;color:#fff;padding:12px 28px;border-radius:30px;font-size:16px;cursor:pointer;transition:.3s;margin:5px}
.btn:hover{transform:scale(1.05);background:#7c3aed}
.btn-secondary{background:#374151}
.btn-secondary:hover{background:#4b5563}
.back{display:inline-block;margin-top:20px;color:#a78bfa;text-decoration:none;border:1px solid #a78bfa;padding:6px 24px;border-radius:30px;transition:.3s}
...
<div class="box">
  ...
  <!-- 图例：去掉二星，只保留三种 -->
  <div class="legend">
    <span class="legend-item"><span class="legend-icon">⭐</span><span class="legend-label">5星</span></span>
    <span class="legend-item"><span class="legend-icon">🌟</span><span class="legend-label">4星</span></span>
    <span class="legend-item"><span class="legend-icon">💫</span><span class="legend-label">3星</span></span>
  </div>
...
<script>
let total=0;
function draw(n){let html='',five=false;for(let i=0;i<n;i++){total++;const r=Math.random();let star='';if(r<0.006){star='⭐';five=true;}else if(r<0.056){star='🌟';}else{star='💫';}html+='<span class="star">'+star+'</span>';}
document.getElementById('result').innerHTML=html;
document.getElementById('count').textContent=total;
if(five)setTimeout(()=>alert('🎉 出五星了！'),100);}
</script>
</body>
</html>`;
}
