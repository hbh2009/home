...
<head>
  ...
  <style>
    ...
    .dark { --bg: #18181b; --card: rgba(39,39,42,0.7); --text: #e4e4e7; --title: #f4f4f5; --border: rgba(82,82,91,0.7); --mask: rgba(0,0,0,0.7); background-blend-mode: multiply; }
    * { margin: 0; padding: 0; box-sizing: border-box; }
    html, body { width: 100%; overflow-x: hidden; }
    body { min-height: 100vh; font-family: -apple-system, BlinkMacSystemFont, "Microsoft YaHei", "Segoe UI", sans-serif; line-height: 2.5; background: var(--bg); background-size: cover; background-position: center; background-attachment: fixed; color: var(--text); transition: all 0.4s ease; padding: 20px 15px; }
    .info-container { width: 100%; max-width: 600px; margin: 0 auto; background: var(--card); padding: 35px 30px; border-radius: 12px; box-shadow: 0 4px 20px rgba(0,0,0,0.1); backdrop-filter: blur(14px); }
    .music-player-wrapper { width: 100%; max-width: 420px; margin: 25px auto 0; }
    #aplayer { max-width: 420px; margin: 20px auto; }
    @media (max-width: 768px) { .music-player-wrapper { padding: 0 10px; margin: 0.8rem auto 0; } html, body { touch-action: manipulation; } }
    ...
    .lang-btn, .theme-btn, .dark-btn, .bg-btn, .xiaohei-toggle-btn, .admin-btn { background: var(--card); border: 1px solid var(--primary); color: var(--primary); padding: 6px 14px; border-radius: 6px; cursor: pointer; font-size: 14px; transition: all 0.2s; font-family: inherit; }
    ...
    .modal-box { background: var(--card); border-radius: 12px; max-width: 500px; width: 100%; padding: 30px 25px; box-shadow: 0 10px 30px rgba(0,0,0,0.2); backdrop-filter: blur(14px); }
    ...
    .form-input { width: 100%; padding: 10px 12px; border: 1px solid var(--border); border-radius: 6px; background: rgba(255,255,255,0.1); color: var(--text); font-size: 14px; font-family: inherit; }
    ...
    .toast { position: fixed; top: 30px; left: 50%; transform: translateX(-50%) translateY(-100px); background: var(--primary); color: #fff; padding: 10px 20px; border-radius: 8px; z-index: 99999; opacity: 0; transition: all 0.4s; }
    .toast.show { opacity: 1; transform: translateX(-50%) translateY(0); }
    .paw { position: fixed; pointer-events: none; z-index: 9999; font-size: 22px; animation: pawAnim 0.9s ease-out forwards; }
    @keyframes pawAnim { 0% { opacity: 1; transform: scale(0.8); } 50% { transform: scale(1.1); } 100% { opacity: 0; transform: scale(0.6) translateY(-12px); } }
    ...
    .quote-toast { position: fixed; bottom: 80px; left: 50%; transform: translateX(-50%) translateY(100px); background: var(--card); border: 1px solid var(--primary); padding: 12px 24px; border-radius: 8px; z-index: 99999; opacity: 0; transition: all 0.4s; max-width: 80%; text-align: center; }
    .quote-toast.show { opacity: 1; transform: translateX(-50%) translateY(0); }
    #xiaohei { position: fixed; width: 65px; height: auto; z-index: 999; right: 20px; bottom: 20px; transition: transform 0.1s; }
...
(function() {
  ...
  const langPack = {
    "zh-CN": { pageTitle: "胡柏珲的个人主页", contact: "联系方式", copyTip: "已复制", adminTitle: "管理员后台", adminPwd: "管理员密码", adminMusic: "网易云音乐歌曲ID", cancel: "取消", save: "保存修改", darkMode: "深色模式", showXiaohei: "显示小黑", hideXiaohei: "隐藏小黑" },
    "zh-TW": { pageTitle: "胡柏珲的個人主頁", contact: "聯絡方式", copyTip: "已複製", adminTitle: "管理員後台", adminPwd: "管理員密碼", adminMusic: "網易雲音樂歌曲ID", cancel: "取消", save: "儲存修改", darkMode: "深色模式", showXiaohei: "顯示小黑", hideXiaohei: "隱藏小黑" }
  };
  ...
  function applyLang() {
    const p = langPack[currentLang]; document.title = p.pageTitle; pageHeader.textContent = p.pageTitle;
    document.querySelector('.contact-title').textContent = p.contact; toggleDark.textContent = p.darkMode;
    copyTipEl.textContent = p.copyTip; document.querySelector('.modal-title').textContent = p.adminTitle;
    document.querySelectorAll('.form-label')[0].textContent = p.adminPwd; document.querySelectorAll('.form-label')[1].textContent = p.adminMusic;
    $('closeModal').textContent = p.cancel; $('saveMusicBtn').textContent = p.save;
    document.querySelectorAll('.lang-btn').forEach(b => b.classList.remove('active')); $(currentLang).classList.add('active');
  ...
  // ====== 管理后台（修复验证） ======
  adminBtn.onclick = () => { adminModal.classList.add('show'); musicIdInput.value = CURRENT_SONG_ID; adminPassword.value = ''; };
  ...
  saveMusicBtn.onclick = async () => {
    ...
    const rawInput = musicIdInput.value.trim();
    if (!pwd) return showToastMsg('请输入管理员密码');
    if (!rawInput) return showToastMsg('请输入歌曲ID');
    
    // 从输入中提取所有数字（兼容纯数字ID和完整URL）
    const digits = rawInput.replace(/[^0-9]/g, '');
    if (digits.length === 0) {
      return showToastMsg('请输入有效的歌曲ID（数字）');
    }
    const finalId = digits;
    
    try {
      const res = await fetch(location.href, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ password: pwd, songId: finalId })
      });
  ...
  async fetch(request, env) {
    ...
    if (request.method === 'POST') {
      try {
        ...
        // 兼容：只要包含数字就提取出来
        const digits = (songId || '').replace(/[^0-9]/g, '');
        if (digits.length === 0) {
